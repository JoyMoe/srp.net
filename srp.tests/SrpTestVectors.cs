...
{
    [Test]
    public void VerifyTestVectorsRfc5054() {
        VerifyTestVectors("rfc5054");
    }

    [Test]
    public void VerifyTestVectorsSrptools() {
        VerifyTestVectors("srptools");
    }

    private void VerifyTestVectors(string fileName) {
        var json        = GetJsonResource(fileName);
        var testVectors = JsonSerializer.Deserialize<TestVectorSet>(json);
