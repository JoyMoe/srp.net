...
public class SrpParameters
{
    ...
    public SrpParameters(
        Func<HashAlgorithm> hashAlgorithmFactory = null,
        string              largeSafePrime       = null,
        string              generator            = null,
        int?                paddedLength         = null,
        SrpRevision?        revision             = null) {
    ...
    /// <param name="paddedLength">The hexadecimal length of N and g.</param>
    /// <param name="revision">Revision of SRP protocol, defaults to 6a if not provided.</param>
    public static SrpParameters Create<T>(
        string       largeSafePrime = null,
        string       generator      = null,
        int?         paddedLength   = null,
        SrpRevision? revision       = null)
        where T : HashAlgorithm {
        var result = new SrpParameters { Hasher = new SrpHash<T>() };
    ...
    public static SrpParameters Create1024<T>()
        where T : HashAlgorithm {
        return Create<T>(SrpConstants.SafePrime1024, SrpConstants.Generator1024);
    }
    ...
    public static SrpParameters Create1536<T>()
        where T : HashAlgorithm {
        return Create<T>(SrpConstants.SafePrime1536, SrpConstants.Generator1536);
    }
    ...
    public static SrpParameters Create2048<T>()
        where T : HashAlgorithm {
        return Create<T>(SrpConstants.SafePrime2048, SrpConstants.Generator2048);
    }
    ...
    public static SrpParameters Create3072<T>()
        where T : HashAlgorithm {
        return Create<T>(SrpConstants.SafePrime3072, SrpConstants.Generator3072);
    }
    ...
    public static SrpParameters Create4096<T>()
        where T : HashAlgorithm {
        return Create<T>(SrpConstants.SafePrime4096, SrpConstants.Generator4096);
    }
    ...
    public static SrpParameters Create6144<T>()
        where T : HashAlgorithm {
        return Create<T>(SrpConstants.SafePrime6144, SrpConstants.Generator6144);
    }
    ...
    public static SrpParameters Create8192<T>()
        where T : HashAlgorithm {
        return Create<T>(SrpConstants.SafePrime8192, SrpConstants.Generator8192);
    }
    ...
    public override string ToString() {
        return $"SrpParameters.Create<{Hasher.AlgorithmName}>(\"{Prime.ToHex()}\", \"{Generator.ToHex()}\")";
    }
}
