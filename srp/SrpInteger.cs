...
public class SrpInteger : IEquatable<SrpInteger>
{
    ...
    private static string NormalizeWhitespace(string hexNumber) {
        return Regex.Replace(hexNumber ?? string.Empty, @"[\s_]", string.Empty);
    }

    /// <summary>
    /// Gets the <see cref="SrpInteger"/> value representing 0.
    /// </summary>
    public static SrpInteger Zero { get; } = new();
    ...
    public SrpInteger Pad(int newLength) {
        return new SrpInteger() {
            Value = Value, HexLength = newLength,
        };
    }

    internal static int Max(params int?[] values) {
        return values.Max(v => v ?? 0);
    }
    ...
    public static SrpInteger RandomInteger(int bytes) {
        if (bytes <= 0) {
            throw new ArgumentException("Integer size in bytes should be positive", nameof(bytes));
        }
    ...
    public SrpInteger ModPow(SrpInteger exponent, SrpInteger modulus) {
        ...
        return new SrpInteger {
            Value = value, HexLength = modulus.HexLength,
        };
    }
    ...
    public static implicit operator string(SrpInteger srpint) {
        return srpint.ToHex();
    }
    ...
    public static implicit operator SrpInteger(int integer) {
        return FromHex(integer.ToString("X"));
    }
    ...
    public static implicit operator SrpInteger(uint integer) {
        return FromHex(integer.ToString("X"));
    }
    ...
    public static implicit operator SrpInteger(long integer) {
        return FromHex(integer.ToString("X"));
    }
    ...
    public static implicit operator SrpInteger(ulong integer) {
        return FromHex(integer.ToString("X"));
    }
    ...
    public static bool operator ==(SrpInteger left, SrpInteger right) {
    ...
    public static SrpInteger operator -(SrpInteger left, SrpInteger right) {
        return new SrpInteger {
            Value = left.Value - right.Value, HexLength = Max(left.HexLength, right.HexLength),
        };
    }
    ...
    public static SrpInteger operator /(SrpInteger dividend, SrpInteger divisor) {
        return new SrpInteger {
            Value = dividend.Value / divisor.Value, HexLength = Max(dividend.HexLength, divisor.HexLength),
        };
    }
    ...
    public static SrpInteger operator %(SrpInteger dividend, SrpInteger modulus) {
        return new SrpInteger {
            Value = dividend.Value % modulus.Value, HexLength = modulus.HexLength,
        };
    }
    ...
    public static SrpInteger operator *(SrpInteger left, SrpInteger right) {
        return new SrpInteger {
            Value = left.Value * right.Value, HexLength = null, // the padding is lost
        };
    }
    ...
    public static SrpInteger FromByteArray(byte[] bytes) {
        ...
        return new SrpInteger {
            Value = value, HexLength = bytes.Length * 2,
        };
    }
    ...
    public override bool Equals(object obj) {
        return Equals(obj as SrpInteger);
    }
